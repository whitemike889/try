...
namespace FibonacciTest
{
    public static class FibonacciGenerator
    {
        ...
        [Fact]
        public async Task Compile_response_contains_entry_point()
        {
            #region bufferSources
...
namespace FibonacciTest
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            var entryPoint = result.GetFeature<CompilationEntryPoint>();
            entryPoint.Should().NotBeNull();
            entryPoint.IsAsync.Should().BeFalse();
            entryPoint.FullyQualifiedName.Should().Match("FibonacciTest.Program.Main");
            entryPoint.Parameters.Length.Should().Be(1);
            entryPoint.Parameters[0].Name.Should().Match("args");
            entryPoint.Parameters[0].Type.Should().Match("string[]");
        }

        [Fact]
        public async Task Compile_response_contains_entry_point_with_typed_arguments()
        {
            #region bufferSources
...
namespace FibonacciTest
{
    public class Program
    {
        public static void Main(string region, string session)
        {
        }
    }
}";
            ...
            entryPoint.IsAsync.Should().BeFalse();
            entryPoint.FullyQualifiedName.Should().Match("FibonacciTest.Program.Main");
            entryPoint.Parameters.Length.Should().Be(2);
            entryPoint.Parameters[0].Name.Should().Match("region");
            entryPoint.Parameters[0].Type.Should().Match("string");

            entryPoint.Parameters[1].Name.Should().Match("session");
            entryPoint.Parameters[1].Type.Should().Match("string");
        }
